...
function applyFilter({
  ...
  const filtered = inputData.filter(item => {
    ...
    if (dateAjoute && (!item.dateAjoute || new Date(item.dateAjoute).toISOString().split('T')[0] !== dateAjoute)) {
      return false;
    }
...
function getItemValue(item: IAIAssistantItem, column: string): string {
  switch (column) {
    case 'name': return item.name?.toString() || '';
    case 'type': return item.type?.toString() || '';
    case 'dateAjoute': return item.dateAjoute
      ? new Date(item.dateAjoute).toISOString().split('T')[0]
      : '';
    case 'educationLevel': return item.educationLevel?.toString() || '';
    case 'subject': return item.subject?.toString() || '';
    case 'chapter': return item.chapter?.toString() || '';
    case 'exercise': return item.exercise?.toString() || '';
    case 'status': return item.status?.toString() || '';
    default: return '';
  }
}