...
export const useQuestionManager = ({
  ...
  const addQuestion = useCallback(
    ...
    [questions, validateQuestion, onQuestionsChange]
  );
  ...
  const updateQuestion = useCallback(
    (questionId: string, updates: Partial<QuestionFormData>) => {
      ...
      const updatedData = { ...currentQuestion, ...updates };
  ...
  const addOption = useCallback(
    (questionId: string) => {
      ...
      const newOption: MultipleChoiceOption = {
        ...
        text: '',
        isCorrect: false,
        order: (question as any).options?.length || 0,
      };

      updateQuestion(questionId, {
        options: [...((question as any).options || []), newOption],
      });
    },
