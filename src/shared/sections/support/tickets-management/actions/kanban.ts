...
export function useGetBoard() {
  ...
  const memoizedValue = useMemo(() => {
    const rawTasks = data?.board.tasks ?? {};
    const columns = data?.board.columns ?? [];
    
    // Transform the tasks to convert assignee arrays to single assignee objects
    const tasks = Object.fromEntries(
      Object.entries(rawTasks).map(([columnId, columnTasks]) => {
        const transformedTasks = columnTasks.map(task => ({
          ...task,
          // Use type assertion to handle the array, then take the first item or null
          assignee: Array.isArray(task.assignee) && task.assignee.length > 0 
            ? task.assignee[0] 
            : task.assignee || null
        }));
        
        return [columnId, transformedTasks];
      })
    );
